...
signal purity, is shown in Table.~\ref{tab:nuel_selection}. Note that
...
\begin{table}
    ...
    {Table showing CC $\nu_{e}$ selected event numbers and efficiencies in brackets for the
        various event categories as well as associated all $\nu_{e}$ (appeared and beam) and
        signal purity. Shown are the post preselection, \emph{cosmic score} cut, and \emph{escapes
            score} cut numbers (Cuts) in addition to the $\mathrm{efficiency}\times\mathrm{purity}$
        optimised (FOM) selection.}
...
\begin{table}
    ...
    {Table showing CC $\nu_{\mu}$ selected event numbers for the various event categories as well
        ...
        preselection (pre), \emph{cosmic score} cut (cos), and \emph{escapes score} cut (esc)
        numbers in addition to the $\mathrm{efficiency}\times\mathrm{purity}$ optimised
        selection.}
...
visible particles in the final state as do CC-DIS events.
...
interaction types can be exploited to improve neutrino energy, charged lepton, and interaction
vertex position and time estimation. For events classified as either CC $\nu_{e}$ or CC
$\nu_{\mu}$ with an associated \emph{CC category} interaction type, the corresponding bespoke
trained network outlined in Section.~\ref{sec:cvn_specific_energy} is used for estimation.

Only three networks for each neutrino type are trained, one for each of the dominant interaction
types CC-QEL(CC-MEC), CC-Res, or CC-DIS. For events not classified by \emph{CC category} to belong
to one of these categories, such as CC-Coh or CC-Other, the CC-Res network is used, due to it
being the most topologically similar interaction type.
...
As the training samples contain a spectra of events typical of the beam, it is important to check
that the CNN neutrino energy predictions are not simply predicting an energy close to the expected
peak beam energy. The probabilities of a CNN estimated neutrino energy given the true neutrino
energy are shown in Fig.~\ref{fig:final_energy_2d} for both CC $\nu_{e}$ and CC $\nu_{\mu}$
events. CNN estimated energy is found to be roughly equal to true energy across the full range of
...
Furthermore, a summary of the neutrino energy resolutions achieved for both CC $\nu_{e}$ and CC
$\nu_{\mu}$ events is shown in Tab.~\ref{tab:energy_resolutions}.

\begin{figure} % FINAL ENERGY FRAC DIAGRAM %
    \includegraphics[width=0.8\textwidth]{diagrams/6-cvn/chipsnet/final_energy_frac.pdf}
    \caption[Distributions of (reco-true)/true neutrino energies for CC $\nu_{e}$ and CC
        $\nu_{\mu}$ events.] {Distributions of (reco-true)/true neutrino energies for both CC
...
in nature, are typically estimated with a neutrino energy lower than their truth value. This is
expected given the missing energy of the final state neutrino. The final neutrino energy
resolutions are comparable to the 7\% signal neutrino energy resolution achieved by
NoVA~\cite{jiang2019}. Additionally, the resolutions follow the expected pattern for the different
interaction types, with the simple to reconstruct single charged lepton QEL interactions achieving
a much better resolution that multi particle DIS events.

The means and standard deviations of gaussian fits made to histograms of ratios of differences
between CNN estimated (reco) and true neutrino energy to true neutrino energy for both CC
$\nu_{e}$ and CC $\nu_{\mu}$ beam events in \unit{1}{GeV} wide bins are shown in
Fig.~\ref{fig:final_energy_nuel} and Fig.~\ref{fig:final_energy_numu} respectively, split by
interaction type.
...
significant for Res and QEL events. This is most likely due to the use of a peaked flux
distribution of training events rather than a flat flux as explored in Ref.~\cite{baldi2019}. The
standard deviation is seen to decrease for all neutrino and interaction types with true neutrino
energy, except for energies above the main flux peak, again likely due to the training sample
used.
...
\begin{figure} % FINAL NUEL ENERGY DIAGRAM %
    ...
    {Means (left) and standard deviations (right) of fits made to distributions of ratios of
        differences between CNN estimated (reco) and true neutrino energy to true neutrino energy
        for CC $\nu_{e}$ events across a range of energy bins and split by interaction type. The
        true distribution of CC $\nu_{e}$ events is shown in green.}
...
\begin{figure} % FINAL NUMU ENERGY DIAGRAM %
    ...
    {Means (left) and standard deviations (right) of fits made to distributions of ratios of
        differences between CNN estimated (reco) and true neutrino energy to true neutrino energy
        for CC $\nu_{\mu}$ events across a range of energy bins and split by interaction type. The
        true distribution of survived CC $\nu_{\mu}$ events is shown in green.}
...
Section.~\ref{sec:cvn_old_reco}. Although the standard reconstruction makes no attempt to estimate
the neutrino energy, the energy of the primary charged lepton in each event is estimated, which
...
true charged lepton energy for both CC $\nu_{e}$ and CC $\nu_{\mu}$ beam events are shown in
Fig.~\ref{fig:final_frac_e_comparison}.

An energy resolution 32\% and 40\% the size of the standard reconstruction resolution is seen for
CNN estimation for CC $\nu_{e}$ and CC $\nu_{\mu}$ QEL events respectively. A very significant
improvement. The charged lepton energy resolutions achieved by the CNN approach are reasonable
close to those achieved by the fiTQun algorithm used by Super-Kamiokande which achieves
approximately 2.5\% resolution for both CC $\nu_{e}$ and CC $\nu_{\mu}$ QEL
events~\cite{jiang2019}. Importantly, the CNN approach works well across a wide range of energies
and event types, while the standard approach only works well mainly for simple QEL events without
multiple particles in the final state.
...
\begin{figure} % FINAL FRAC E COMPARISON DIAGRAM %
    ...
    {Distributions of (reco-true)/true primary charged lepton energies for both CC $\nu_{e}$
        (left) and CC $\nu_{\mu}$ (right) beam events for both the new CNN approach and standard (old)
        methods. Only QEL events are shown for clearer comparison with the standard reconstruction
        methods. The standard deviation of a gaussian fit made to each distribution is shown in the
        corresponding brackets.}
    \label{fig:final_frac_e_comparison}
\end{figure}

The energy estimation networks also output the interaction vertex position and time. The CNN
estimated (reco) minus truth distributions are shown for CC $\nu_{e}$ and CC $\nu_{\mu}$ QEL
events in Fig.~\ref{fig:final_vertex_nuel_res_comparison} and
Fig.~\ref{fig:final_vertex_numu_res_comparison} respectively, with the standard reconstruction
method distributions alo shown. Similar performance when compared to the standard methods is seen
across all parameters. The Super-Kamiokande fiTQun algorithm achieves a \unit{20}{\mathrm{cm}}
position resolution for CC $\nu_{e}$ QEL events and a \unit{20}{\mathrm{cm}} resolution for CC
$\nu_{\mu}$ QEL events, compared to the approximately \unit{50}{\mathrm{cm}} and
\unit{70}{\mathrm{cm}} resolution achieved here.
...
By combining the CC $\nu_{e}$ and CC $\nu_{\mu}$ selections with the corresponding energy
estimation, the final selected spectrum of events for each within \chipsfive running for a year
can be estimated. These are shown in Fig.~\ref{fig:final_nuel_passed_energy_dist} and
Fig.~\ref{fig:final_numu_passed_energy_dist} for CC $\nu_{e}$ and CC $\nu_{\mu}$ selections
respectively. Although, a detailed \chipsfive sensitivity analysis is not included in this work,
the expected curves for different values of $\delta_{CP}$ are shown in the CC $\nu_{e}$ case for
interest.
...
A major concern with deep neural networks is that they become a black box which is not fully
understood. This has significant implications for detailed physics analyses that require a full
understanding to accurately gauge amongst other things systematic uncertainties to provide
confidence in the final results. Here we detail a sample of efforts to \emph{explain} the inner
workings of the trained CNNs.
...
\begin{figure} % ACTIVATIONS DIAGRAM %
    ...
    {Visualisations of the feature map outputs using as input the event shown in
        ...
        (top), second (middle), and third (bottom) VGG blocks for a chipsnet architecture network
        with only a single branch and all three input event maps stacked into a single three
        channel input image. }
\end{figure}
