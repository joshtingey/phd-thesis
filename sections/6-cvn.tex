...
was \emph{viewed} from its estimated interaction vertex position. The primary motivation behind
...
\begin{table}
    ...
    \caption{Table showing the cap-points (maximum value of the encoding range) and the associated
        ...
        The cap-points are specifically chosen to keep the \emph{capped percentage} to
        approximately 0.1\% as a greater value could lead to the loss of powerful information
        within the data.}
    \label{tab:encoding}
\end{table}
...
major differences from the literature defined network, these are discussed below.
...
\begin{figure} % CHIPSNET DIAGRAM %
    ...
    {Illustrative diagram of the baseline chipsnet architecture. The three input event maps are
        separately passed through two VGG blocks before their outputs being combined and passed
        ...
        with five seed parameters and passed through two fully-connected layers of 512 neurons
        each before the output layer.}
    \label{fig:chipsnet}
\end{figure}
...
- Exact training sample size and make up will be detailed in the network specific sections.
- Training times will be detailed in network specific sections.
- Training took place on a 36 CPU core machine with four Nvidia GeForce RTX 2080 graphics cards each
with 8Gb of on-board memory.
- Data was stored locally on the machines so it could be loaded faster at training time.
- Used Tensorflow dataset API to create efficient input pipeline for loading and using training
data at training time.
- Instead of all being loaded into memory beforehand which would be impossible, data is loaded
on-the-fly at training time.
- Makes use of all four graphics processing units (GPUs).
- The machines CPU cores are used to load, decode and prepare data for training before it is
needed by the GPUs to maximise the efficiency with which they are being used.
- Input data is converted to a float between zero and one.
- We also apply a random factor scaling of every bin in every event map, centred on one with a
standard deviation of $\sigma_{a}$ which is another hyperparameter.
- We varied bin (pixel) intensities by adding gaussian noise with a standard dev of 2\%.
- Clip any negative value to zero.
- This provides augmentation and is another regularisation strategy to prevent overfitting, by
forcing the network to focus less on the absolute bin values and more on the topology of the event.
- Split training data 95\% to 5\% training/validation.
- Minibatch training strategy with a minibatch size $n_{b}$
- We use mixed precision operations where some layers use float-16 operations instead of float-32
operations which speeds up training and is found to not impact performance.
- The learning rate reduces at each epoch (as it gets closer to the minimum) using
\begin{equation}
    \eta_{e}=\frac{\eta_{0}}{1+c_{d}e}
\end{equation}
where $c_{d}$ is the coefficient of decay set to be 0.5 and $e$ is the epoch number
- We implement early stopping, the specific values will be given in the network specific sections.
- Therefore have the following hyperparameters, which are optimised for the final three models
using SHERPA hyperparameter tuning framework which was used for hyperparameter tuning.

\begin{itemize}
    \item \textbf{Initial learning rate, $\eta_{0}$:} range from 0.00005 to 0.001.
    \item \textbf{Learning rate decay coefficient, $c_{d}$:} range from 0.2 to 0.8.
    \item \textbf{Dropout probability, $p_{d}$:} range from 0.0 to 0.5.
    \item \textbf{Augmentation size, $\sigma_{a}$:} range from 0.0 to 0.1.
    \item \textbf{Minibatch size, $n_{b}$:} choice from 32, 64, 128, or 256.
    \item \textbf{Multi-task loss combination strategy:} choice from simple or learnt.
\end{itemize}
