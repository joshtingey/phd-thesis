...
\begin{figure} % CHIPSNET DIAGRAM %
    \includegraphics[width=0.7\textwidth]{diagrams/6-cvn/chipsnet.pdf}emph
...
Here the performance of the CNN approach is presented and compared to that achieved by standard
methods outlined in Section.~\ref{sec:cvn_old} as well as comparable experiments. By combining
cosmic rejection, containment, beam classification, and energy estimation the final selected and
reconstructed signal samples are presented.

Beforehand, however, one key advantage of the CNN approach must be mentioned. Although the time
taken to train all the CNNs required in this work is approximately two full days, once trained the
time taken to fully reconstruct (calculate outputs from all networks) is on the order of
\unit{100}{\mathrm{ms}}.

Here the performance is shown, but first one of the main advantages, BIG IT UP!

Although the time taken to train all CNNs required is approximately two days, once trained the
time taken to calculate the network outputs (inference time) for all of the networks is on the
order of \unit{2}{\mathrm{ms}}, combined with initial event interaction vertex estimation and
event map generation the time taken to fully classify and reconstruct an event is still less than
one second.

This has far reaching implications for the quick iteration of analysis
...
detector, using the expected unoscillated flux, cross-sections, and standard oscillation
...
0.0001 is chosen by inspection to accept beam like events. Out of the total 350000 cosmic events
in the evaluation sample, all are rejected by this cut along with preselection. Without the
addition of the second \emph{escapes score} output, a trained cosmic rejection network instead
allow five cosmic events to pass preselection and the cut above.
...
\begin{figure} % COSMIC OUTPUTS ZOOMED DIAGRAM %
    ...
    {Distribution of \emph{cosmic score} output values from the trained cosmic rejection network
        ...
        is shown with the arrow indicating the events that are selected. No cosmic events are
        shown due to the statistical limitations of the evaluation sample.}
...
to select events. Although this only considers the primary charged lepton instead of all event
...
correctly, resulting in a 95\% purity for the selected CC $\nu_{\mu}$ sample. As expected, the
...
\begin{table}
    ...
    {The total number of events and the number that pass successive selection cuts for the
        different event categories. The preselection, \emph{cosmic score}, and \emph{escapes
...
consider the more statistically significant number that passes a loose \emph{cosmic score} cut
below a value of 0.9. In this case, 501 out of 350000 evaluation sample cosmic events are
selected, which when weighted corresponds to 2384 events per year. After the preselection and
\emph{escapes score} cuts have also been applied only 38 or when weighted 180 cosmic events
survive, corresponding to a rejection factor of $1.5\times10^{-4}$.
...
reality, both are likely to be lower. This is comparable to the approximately $2\%$ cosmic
contamination of the CC $\nu_{\mu}$ sample seen by NoVA~\cite{acero2019}. Due to the constraints
of the evaluation sample outlined above, we do not consider cosmic events for the rest of this
section.
...
events with a score above this optimised value are then accepted as signal. In this work, the
figure-of-merit $\mathrm{efficiency}\times\mathrm{purity}$ is optimised for an efficient sample.
Below the results for both appeared CC $\nu_{e}$ and survived CC $\nu_{\mu}$ selections are
presented.
...
both CC $\nu_{\mu}$ and NC background events is achieved, especially given the large discrepancy
in sample size. As no attempt is made to separate the appeared CC $\nu_{e}$ signal component from
the intrinsic beam CC $\nu_{e}$ background, both are clustered with scores close to one as
expected.
...
corresponding number of selected events for each event category, as well as the signal efficiency
and purity, is shown in Table.~\ref{tab:nuel_selection}. Note that
...
\begin{table}
    ...
    {Table showing CC $\nu_{e}$ selected event numbers for the various event categories as well as
        ...
        \emph{cosmic score}, and \emph{escapes score} numbers in addition to the
...
the appeared CC $\nu_{e}$ purity is observed to peak at approximately \unit{2.5}{\GeV} before
declining. The NC efficiency is seen to slowly increase approaching 15\% for hadronic system
energies above \unit{5}{\GeV}; this is likely due to high energy pions or protons being
...
comparison with the standard selection presented in Section.~\ref{sec:cvn_old_pid}. The
distribution of output scores for both neural networks used in the standard selection is shown in
...
v$\nu_{e}$ selection sample is found. A maximum $\mathrm{efficiency}\times\mathrm{purity}$ of
0.132 is achieved; only 25\% of the value reached by the CNN approach. Although a similar signal
purity (39\%) is achieved, this is associated with a signal efficiency of 34\% compared to the
71\% achieved by the trained CNN. It is clear, therefore, that the new approach provides
considerable improvements to beam classification performance.
...
\begin{figure} % FINAL OLD PID OUTPUTS DIAGRAM %
    ...
    {Distributions of CC $\nu_{e}$ vs CC $\nu_{\mu}$ (left) and CC $\nu_{e}$ vs NC (right) output
        ...
        plot shows events which have passed both a simple preselection and the optimised cut from
        the other network. Selected CC $\nu_{e}$ events are shown by the arrows. The y-axis for
        the CC $\nu_{e}$ vs CC $\nu_{\mu}$ distributions has been truncated so that the CC
        $\nu_{\mu}$ component is not fully visible in order to better show the distribution of
        signal CC $\nu_{e}$ events. Due to the long reconstruction time required, a smaller but
        comparable evaluation sample is used here.}
...
corresponding number of selected events for each event category, as well as the signal efficiency
and purity, is shown in Table.~\ref{tab:numu_selection}. Although the final selection efficiency
is low for both figure-of-merit optimisations, this is desirable to ensure events are fully
contained for energy estimation. When considering just the CC $\nu_{\mu}$ events for which the
primary charged muon is contained, an 87\% selection efficiency for the first figure-of-merit is
achieved.
...
\begin{table}
    ...
    {Table showing CC $\nu_{\mu}$ selected event numbers for the various event categories as well
        ...
        preselection, \emph{cosmic score}, and \emph{escapes score} numbers in addition to the
        $\mathrm{efficiency}\times\mathrm{purity}$ optimised selection.}
...
Fig.~\ref{fig:final_cc_cat_confusion}.
...
Reasonable classification efficiency greater than 60\% is achieved across the three dominant
interaction types CC-QEL, CC-Res, and CC-DIS. The less common CC-Coh and CC-MEC types are found to
be commonly misidentified as CC-Res and CC-QEL respectively, most likely due to the imbalanced
...
typically classified as CC-DIS in nature; this is expected as they commonly contain multiple
visible particles in the final state as do DIS events.
...
well, with NC-Coh events typically being classified as resonant. Of the CC events that are not
...
By using the CC interaction type classification just presented, we can exploit
...
- Can exploit the ability to separate CC interaction types to better estimate the neutrino energy
of events.
- MEC and QEL are trained combined anyway, Coh classified used resonant trained network, these are
small contributors to the total flux so their contribution to the final resolutions are
negligible.
- Other we use res as well, why not!!!
...
The distributions of true and CNN estimated $\nu_{e}$ and $\nu_{\mu}$ energies for true CC
$\nu_{e}$ and CC $\nu_{\mu}$ events respectively that are also selected by their corresponding CC
selection are shown in Fig.~\ref{fig:final_energy_dists}. The CNN estimated distributions match
the truth well across the full range of neutrino energies expected within \chipsfive, except
perhaps in the peak regions where the exact truth distribution shape is not fully captured.
...
As all energy estimation network training samples contain a spectra of events typical of the beam,
it is important to check that the CNN neutrino energy predictions are not simply predicting an
energy close to the expected peak energy. The probabilities of a CNN estimated neutrino energy
given the true neutrino energy are shown in Fig.~\ref{fig:final_energy_2d} for both CC $\nu_{e}$
and CC $\nu_{\mu}$ events. CNN estimated energy is found to be roughly equal to true energy across
the full range of expected \chipsfive beam energies, showing the networks learn as desired.
...
\begin{figure} % FINAL 2D ENERGY DIAGRAM %
    ...
    {CNN estimated vs true neutrino energy for CC $\nu_{e}$ (left) and CC $\nu_{\mu}$ (right) beam
        ...
        normalised to better show the regions outside dominant 2 to \unit{4}{\GeV} range. Only
        true CC $\nu_{e}$ and CC $\nu_{\mu}$ events that are also selected by the CC $\nu_{e}$ and
        CC $\nu_{\mu}$ selections respectively are shown.}
    \label{fig:final_energy_2d}
\end{figure}

To understand CNN energy estimation performance, histograms of ratios of differences between CNN
estimated (reco) and true neutrino energy to true neutrino energy for both CC $\nu_{e}$ and CC
...
beam selection, detailed in the previous section. Similar distributions splitting the
\emph{signal} component by interaction type are shown in Fig.~\ref{fig:final_energy_frac_split}.

A summary of the neutrino energy resolutions achieved for both CC $\nu_{e}$ and CC $\nu_{\mu}$
events is shown in Tab.~\ref{tab:energy_resolutions}. The tail of negative values for the
\emph{all} selected CC $\nu_{e}$ distribution indicates that wrongly classified CC $\nu_{e}$
events, mainly NC in nature, are typically estimated with a neutrino energy lower than their truth
value. This is expected given the missing energy of the final state neutrino which is not
detected. Values are comparable to the approximately 7\% signal neutrino energy resolution NoVA
achieves~\cite{jiang2019}.
...
\begin{figure} % FINAL ENERGY FRAC DIAGRAM %
    ...
    \caption[Distributions of ratios of differences between CNN estimated and true neutrino energy
        to true neutrino energy.] {Distributions of (reco-true)/true neutrino energies for both CC
        $\nu_{e}$ (left) and CC $\nu_{\mu}$ (right) beam events. Distributions for both \emph{all}
        respectively selected events and just the \emph{signal} component of each selection are
        shown.}
...
\begin{table}
    ...
    {Summary of CC $\nu_{e}$ and CC $\nu_{\mu}$ neutrino energy resolutions. Shown for each sample
        are the resolutions for \emph{all} selected events, the true \emph{signal} selected events and
        the three dominant \emph{signal} interaction type components, QEL, Res, and DIS. The
...
are shown in Fig.~\ref{fig:final_energy_nuel} and Fig.~\ref{fig:final_energy_numu} respectively.
...
\begin{figure} % FINAL VTX RES NUEL COMPARISON DIAGRAM %
    ...
    \caption[final vertex nuel res comparison short]
    {final vertex nuel res comparison long}
...
\begin{figure} % FINAL VTX RES NUMU COMPARISON DIAGRAM %
    ...
    \caption[final vertex numu res comparison short]
    {final vertex numu res comparison long}
...
The final selected number of events within \chipsfive after a year of running for both CC
$\nu_{e}$ and CC $\nu_{\mu}$ selected samples are shown in
...
\begin{figure} % EXPLAIN EXAMPLE EVENT DIAGRAM %
    ...
    {Three map representation of a CC quasi-elastic $\nu_{e}$ event. Initiated by a $\nu_{e}$
        of energy \unit{2.4}{\GeV} with a final state $e^{-}$ of energy \unit{1.6}{\GeV}.}
...
\begin{figure} % ACTIVATIONS DIAGRAM %
    ...
    {Visualisations of the feature map outputs using as input the event shown in
        Fig.~\ref{fig:explain_example_event}.}
\end{figure}
...
- t-Distributed Stochastic Neighbour embedding, an unsupervised learning technique to visualise
the high dimensional network feature-space (in this case the final fully-connected layer) in two
dimensions.
- Shows how the features the network has learnt are able to discriminate between categories.
Events with similar learnt features are clustered together close in the two dimensional t-SNE
space. Can see how well the different categories have been separated.
