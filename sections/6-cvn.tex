...
For the majority of HEP experiments, event analysis aims to separate signal from background,
identify particles types, discover kinematic properties and estimate energies. The same is true
for \chips, with the primary aims being the selection of CC $\nu_{e}$ signal events from a
sizeable background and estimation of the associated neutrino energy.
...
Monte Carlo simulation. The last listed input is comparable to that shown in
...
dataset are inferred without example data is not discussed, however, will be used for
\emph{explainability} in Section.~\ref{sec:cvn_explain}.
...
efficiency and 57-78\% purity (depending on oscillation parameters) achieved by the \nova appeared
