...
evaluation sample contains the expected combination of underlying beam interaction types, as shown
in Fig.~\ref{fig:explore_stacked_int_types}.
...
\begin{figure} % OSC FLUXES DIAGRAM %
    ...
    {The weighted spectrum of events contained within the evaluation sample. The weighting is
        designed to mimic the expected event spectrum of the \chips detector. Beam events are
        ...
        the energy visible to the detector better. Finally, shown in black is the cosmic muon
...
Separate from any CNN driven work a simple preselection is applied to all events in the evaluation
...
this, a `cosmic score' value above 0.0001 is chosen by inspection to reject cosmic like events.
Out of the total 350000 cosmic events in the evaluation sample, all are rejected by this cut.
...
particularly important for CC $\nu_{\mu}$ events for which only 44\% of the primary charged muons
are fully contained within the detector volume.
...
particles, it still acts as a reasonably simplified proxy from event containment. The distribution
...
selected with an efficiency greater than 90\%, while $\nu_{\mu}$ events are heavily reduced due to
their containment. Furthermore, NC events are primarily rejected by the preselection, while cosmic
events are heavily rejected by both the preselection and `cosmic score' cuts.
...
350000 evaluation events that pass the cuts is not statistically significant. However, due to
computational constraints, a sufficiently sized cosmic evaluation sample is deemed infeasible for
this work.
...
rejection factor for cosmic muon events of $1.5\times10^{-4}$.

Furthermore of the surviving 180 cosmic events, 95 would be selected as CC $\nu_{\mu}$ events and
zero as CC $\nu_{e}$ events by the beam classification detailed in the following section. This
...
reality, both are likely to be lower. Due to the constraints of the evaluation sample outlined
above, we do not consider cosmic events for the rest of this section.
...
network give the probability score that the event in question belongs to the corresponding
category. As the output neuron scores collectively sum to one, the highest scoring neuron can be
used to simply classify events as either CC $\nu_{e}$, CC $\nu_{\mu}$, or NC in nature.
...
two figure-of-merit values are optimised, $\mathrm{efficiency}\times\mathrm{purity}$ and $s/\sqrt{b}$, where $s$
and $b$ are the number of selected signal and background events respectively. The first optimises
for an efficient sample, while the second optimises for a pure sample. Below are the results for
both CC $\nu_{e}$ and CC $\nu_{e\mu}$ selections.

\subsubsection*{CC $\nu_{e}$ selection} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The distribution of output CC $\nu_{e}$ scores for the different event categories is shown in
Fig.~\ref{fig:final_beam_nuel_outputs}. Excellent separation between appeared CC $\nu_{e}$ signal
...
component from the intrinsic beam CC $\nu_{e}$ background, both are clustered with event scores
...
The efficiency, purity, and their product for all CC $\nu_{e}$ events (both appeared and beam) as
a function of selecting events above a certain CC $\nu_{e}$ score value is shown in
Fig.~\ref{fig:final_nuel_eff_curves}. The maximum $\mathrm{efficiency}\times\mathrm{purity}$
value, the first of the two figure-of-merits, is found at a CC $\nu_{e}$ score of 0.8 as can be
seen. The number of selected events for each event category and corresponding signal efficiency
and purity for both optimised figure-of-merits is shown in Table.~\ref{tab:nuel_selection}.
...
\begin{table}
    ...
    {Table showing CC $\nu_{e}$ selected event numbers for the various event categories as well as
        ...
        figure-of-merit optimisations for which the cut value is also given in brackets.}
