...
construction on land. The construction site is easily accessible by road and well connected to
power due to the heavy infrastructure in place for mining. Additionally, the nearby PolyMet mining
administration building is used as a laboratory environment for the construction and testing of
...
\begin{figure} % CHIPS FRAME DIAGRAM %
    ...
    {Picture of the \chipsfive structural frame  with humans for scale. The endcaps can be seen
        ...
        inside of each endcap to mount the instrumentation to.}
    \label{fig:frame}
\end{figure}
...
beam to furthermore increase light collection.
...
pigtail ensures that even if the outside connection is flooded, every POM is capable of
...
when only studying accelerator beam neutrinos. However, importantly, photocathode coverage in the
upstream regions of the detector is still required, even if very low, in order to reject cosmic
muon and NC events.
...
installed to constantly pull detector water through a \unit{500}{\mathrm{m}} long flexible HDPE
pipe to a filtration hut on shore. After filtration the water is returned to the detector through
a second identical pipe.
...
out by a team of approximately 10 to 15 collaborators at any one time. Given the novel nature of
the project, most tasks, some of which are pictured in Fig.~\ref{fig:work1} and
Fig.~\ref{fig:work2}, proved challenging. Interestingly, an ability to derive pleasure from
permanently wet feet was found to be invaluable for personal morale.
...
2020 were suspended, including important repairs and instrumentation of the barrel walls. Plans
...
detector simulation used by \chips are described. All are employed extensively for work presented
...
consisting of two endcaps and a barrel, filled with water and lined with a low reflectivity
\emph{blacksheet}. The geometry is separated into \emph{regions} within both the barrel and
endcaps, defined either by a list of barrel sides or an opening angle respectively. Each region is
filled with a unique base unit of geometry known as the \emph{unit cell} as shown in
Fig.~\ref{fig:sim_geom}.
...
simulation) they can be scaled by values within yet another configuration file. This scaling
controls the water absorption and scattering (Rayleigh and Mie) lengths, and both the blacksheet
and PMT glass reflectivity. In this work an attenuation length of \unit{50}{\mathrm{m}} at
...
smeared first hit time used as the recorded time. A method similar to that employed in the
Super-Kamiokande simulation determines the total output charged of the hit given the number of
incident photons. A single photoelectron charge distribution is repeatedly probed for each photon
and the combined sum returned. By sampling this procedure multiple times, the output charge
probability distribution given the number of incident photons is shown in
Fig.~\ref{fig:digitisation}. The figure also shows the reverse \emph{likelihood} function (lower
is more likely) of an actual number of incident photons given a measured digitised charge.
