...
\numi beam achieves an intensity of \unit{700}{\text{kW}} (\unit{740}{\text{kW}} at peak)
...
Every \unit{1.33}{\text{seconds}} a \unit{10}{\mu\text{s}} long spill of protons accelerated
...
hadrons are allowed to decay in flight to a beam of muon neutrinos in a \unit{675}{\text{m}}
...
downstream of \unit{300}{\text{m}} of rock: The MINERvA spectrometer~\cite{mcfarland2006}, the
...
the \unit{7}{\text{mrad}} off-axis \chipsfive module.
...
\begin{figure} % OFF-AXIS FLUX DIAGRAM %
    ...
    {Muon neutrino flux for different \numi detectors at different off-axis angles. Shown are the
        neutrino energy spectrum for MINOS (on-axis), \nova (\unit{14}{\text{mrad}} off-axis),
        and \chipsfive (\unit{7}{\text{mrad}} off-axis). Figure taken from
...
\end{equation}
Again using $n\sim 1.33$, a threshold energy of $\sim$\unit{1.5}{\text{m}} is typical.
...
range of wavelengths for which PMTs are typically sensitive, \unit{350}{\text{nm}} to
\unit{650}{\text{nm}}, and using $\beta\sim 1$ and $n\sim 1.33$ gives approximately $240$
...
\unit{25}{\text{m}} wide and \unit{12}{\text{m}} high, \chipsfive is cylindrical in shape with
an inner surface area of \unit{1924}{\text{m}^2} and a total target mass of
\unit{5.9}{\text{kton}}. Via the process of design, construction, deployment, and data taking,
\chipsfive primarily aims to refine the \chips concept for future full-scale
($\sim$\unit{15}{\text{kton}}) modules. Consequently, \chipsfive is designed such that the
...
is located \unit{7}{\text{mrad}} off the \numi axis at a distance of \unit{712}{\text{km}}
from the beam target. Roughly \unit{0.8}{\text{km}}$\times$\unit{1.2}{\text{km}} in size with
a maximum depth of \unit{60}{\text{m}} ($\pm$\unit{3}{\text{m}} throughout the year), the pit
allows for a water overburden of approximately \unit{50}{\text{m}} with \chipsfive resting on
...
A sizeable earthen ramp on the south side of Wentworth 2W is used for detector construction while
...
named \emph{top-cap} is held above the \emph{bottom-cap} by \unit{1.5}{\text{m}} long steel
...
The two endcaps are connected by 28 \unit{12}{\text{m}} long Dyneema cables around their
perimeter. Additionally, 48 \unit{16}{\text{inch}} diameter air-filled PVC pipes are attached to
...
\unit{2}{\text{m}}$\times$\unit{3}{\text{m}} array of watertight PVC tubing equipped with
...
Firstly, \emph{Nikhef} POMs use \unit{88}{\text{mm}} HZC PMTs with electronics developed by the
KM3NeT experiment~\cite{katz2009, adrian2016}. Secondly, \emph{Madison} POMs use
\unit{3}{\text{inch}} Hamamatsu R6091 PMTs donated from the NEMO 3 experiment~\cite{arnold2005}
with novel electronics developed by \chips in collaboration with the Wisconsin IceCube Particle
Astrophysics Centre (WIPAC) in Madison, Wisconsin. The \unit{88}{\text{mm}} HZC PMTs have a high
ratio of output electrons to incident photons (quantum efficiency) of 24.4\% at a wavelength of
\unit{400}{\text{nm}}, compared to the low 12.0\% ratio achieved by the R6091 PMTs. Furthermore,
the photon hit time resolution is $\sim$\unit{2}{\text{ns}} and $\sim$\unit{5}{\text{ns}} for
the \unit{88}{\text{mm}} HZC and R6091 PMTs respectively.

In total $6114$ \unit{88}{\text{mm}} HZC, and $450$ Hamamatsu R6091 PMTs are arranged into $226$
...
the \unit{6}{\text{atm}} of water pressure at the bottom of the pit. A fully assembled and
...
\begin{figure} % NIKHEF PMT ASSEMBLY DIAGRAM %
    ...
    {Disassembled (a) and assembled (b) Nikhef PMT assembly components. The assembly comprises of
        a black PVC insert, a \unit{88}{\text{mm}} HZC PMT, a transparent acrylic cover, and a
...
\begin{figure} % MADISON PMT ASSEMBLY DIAGRAM %
    ...
    {A Hamamatsu R6091 Madison POM PMT outside (a) and inside its insert (b). The PMT is
        ...
        the \unit{6}{\text{atm}} of water pressure at the bottom of the pit. A microDAQ detailed
...
Firstly, a \emph{full-density} Nikhef POM region in the most downstream $\phi=\pm75^{\circ}$
...
geomembrane liner, the \unit{1.3}{\text{m}} high region allows for the rejection of
predominantly downward going cosmic muons by detecting the Cherenkov radiation they produce. A
total of $324$ upward-facing \unit{88}{\text{mm}} HZC veto PMTs within the top-cap Nikhef POMs
...
through a \unit{500}{\text{m}} long flexible PVC umbilical resting on the pit floor. The
...
necessary \unit{30}{\text{m}} photon attenuation length. Therefore, a high volume pump is
installed to constantly pull detector water through a \unit{500}{\text{m}} long flexible HDPE
...
consists of a 20 inch \unit{10}{\mu\text{m}} carbon block filter followed by a 20 inch
\unit{0.5}{\mu\text{m}} polypropylene filter, as shown in fig.~\ref{fig:filtration}. This
...
height to \unit{8}{\text{m}}. In October 2019 this version of \chipsfive (shown in
...
\begin{figure} % COMPLETE PICTURE DIAGRAM %
    \includegraphics[width=\textwidth]{diagrams/4-chips/complete.pdf}
...
reflectivity. In this work a photon attenuation length of \unit{50}{\text{m}} at
\unit{405}{\text{nm}} is used with negligible scattering, the blacksheet reflectivity is set to
...
inner detector volume, while cosmic vertices are kept at \unit{1}{\text{m}} above the detector
...
\unit{200}{\text{ns}} windows are grouped to form a single recorded hit, with a smeared first
...
information and track descriptions. A full event takes approximately \unit{3}{\text{seconds}} to
...
\unit{1}{\text{km}}$\times$\unit{1}{\text{km}} area, with the detector at it's centre. Note
that only single muon events (the dominant cosmic component) are considered for simplicity.

Assuming a \chipsfive overburden of \unit{50}{\text{m}} and a \unit{2.2}{\MeV/\text{cm}^{2}}
muon energy loss in water as suggested by Ref.~\cite{klimushin2001} the muon parameters are
updated to estimate their values \unit{1}{\text{m}} above the top of the detector. All muons
...
\unit{24}{\text{m}} are used to estimate a \chipsfive cosmic muon rate of
\unit{11.8}{\text{KHz}} at \unit{50}{\text{m}} of overburden.

Given the \unit{10}{\mu\text{s}} long \numi beam spill occurring every \unit{1.33}{\text{s}}
an in spill cosmic rate of $\sim2.1$ million events per year is calculated with an in spill
occupancy of 9\%. Considering a typical event takes $\sim$\unit{100}{\text{ns}} to unfold, there
...
\begin{figure} % COSMIC RATE DIAGRAM %
    ...
    {Expected cosmic muon rate as a function of water overburden depth for a \unit{24}{\text{m}}
        high and \unit{24}{\text{m}} wide \chips detector module. Shown are fits made to the
