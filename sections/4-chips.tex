...
\begin{itemize}
    \item \textbf{Detector construction:} Aiming to prove that the construction and deployment of
          ...
          pfutznerProto2017, pfutzner2017}. Secondly, the much larger \SI{5}{\text{kt}} \chipsfive
    ...
    \item \textbf{Water filtration:} Aiming to prove that adequate water purity can be achieved
          ...
          mine pits), adequate photon attenuation lengths of greater than \SI{100}{\text{m}} are
...
\begin{figure} % CHIPS-M DIAGRAM %
    ...
    {Picture of the \SI{3.3}{\text{m}} high \chipsm detector just before deployment. Temporary
...
\numi beam achieves an intensity of \SI{700}{\text{kW}} (\SI{740}{\text{kW}} at peak) making it
the most powerful such beam in the world. A schematic of the \numi beamline configuration is shown
in \FigureRef{fig:numi_beam}.
...
Every \SI{1.33}{\text{seconds}} a \SI{10}{\micro\text{s}} long spill of protons accelerated to
...
\SI{675}{\text{m}} long decay pipe via the processes:
...
\SI{300}{\text{m}} of rock: The MINERvA spectrometer~\cite{mcfarland2006}, the near detector for
...
reduced, as is the case for the \SI{7}{\text{mrad}} off-axis \chipsfive detector module.
...
\begin{figure} % OFF-AXIS FLUX DIAGRAM %
    ...
    {Muon neutrino flux for different \numi detectors at different off-axis angles. Shown are the
        neutrino energy spectrums for MINOS (on-axis), \chipsfive (\SI{7}{\text{mrad}} off-axis),
        and \nova (\SI{14}{\text{mrad}} off-axis). Figure taken from \ReferenceRef{adamson2013}.}
    \label{fig:numi_axis}
\end{figure}
...
\begin{figure} % CHERENKOV EFFECT DIAGRAM DIAGRAM %
    ...
    {Diagram of Cherenkov radiation emission and wavefront angles. In a dispersive medium photons
        ...
        not equal to $\theta_{c}$. Figure taken from \ReferenceRef{particle2020}}
    \label{fig:cherenkov}
\end{figure}
...
the range of wavelengths for which PMTs are typically sensitive, \SI{350}{\text{nm}} to
\SI{650}{\text{nm}}, and using $\beta\sim 1$ and $n\sim 1.33$ gives approximately $240$ photons
...
in shape with dimensions of \SI{25}{\text{m}} in diameter and \SI{12}{\text{m}} in height,
\chipsfive has an inner surface area of \SI{1924}{\text{m}^2} and a total target mass of
\SI{5.9}{\text{kt}}. Via the process of design, construction, deployment, and data taking,
\chipsfive primarily aims to refine the \chips concept for future full-scale
($\sim$\SI{15}{\text{kt}}) modules. Consequently, \chipsfive is designed such that the details
...
is located \SI{7}{\text{mrad}} off the \numi axis at a distance of \SI{712}{\text{km}} from the
beam target. Roughly \SI{0.8}{\text{km}} by \SI{1.2}{\text{km}} in size with a maximum depth of
\SI{60}{\text{m}} ($\pm$\SI{3}{\text{m}} throughout the year), the pit allows for a water
overburden of approximately \SI{50}{\text{m}} above \chipsfive when resting on the bottom. With an
...
The structure of \chipsfive consists primarily of two \SI{26}{\text{m}} diameter and
\SI{1.3}{\text{m}} high lightweight stainless steel circular \emph{endcaps} that form the top and
bottom of its cylindrical shape. During construction the conveniently named \emph{top-cap} is held
above the \emph{bottom-cap} by \SI{1.5}{\text{m}} long steel struts as shown in
...
\SI{12}{\text{m}} in length. Additionally, a total of 48 air-filled Polyvinyl Chloride (PVC)
pipes, each \SI{40}{\text{cm}} in diameter, are attached to the frame of the top-cap to make it
...
IceCube and KM3NeT~\cite{hanson2006, eijk2015}. Each \textsc{Pom} is a roughly \SI{2}{\text{m}} by
\SI{3}{\text{m}} array of watertight PVC tubing equipped with between $15$ to $30$ PMTs, as well
...
Firstly, \emph{Nikhef} \textsc{Pom}s use \SI{88}{\text{mm}} in diameter HZC PMTs~\cite{hzc}, with
electronics developed for the KM3NeT experiment~\cite{katz2009, adrian2016}. Secondly,
\emph{Madison} \textsc{Pom}s use \SI{76}{\text{mm}} in diameter Hamamatsu PMTs~\cite{hamamatsu},
...
24.4\% at a wavelength of \SI{400}{\text{nm}}, compared to the low 12.0\% ratio achieved by the
Hamamatsu PMTs. Furthermore, the photon hit time resolution is $\sim$\SI{2}{\text{ns}} and
$\sim$\SI{5}{\text{ns}} for the HZC and Hamamatsu PMTs respectively.
...
becomes flooded, the other is still capable of withstanding the \SI{6}{\text{atm}} of water
...
\begin{figure} % MADISON PMT ASSEMBLY DIAGRAM %
    ...
    {A Hamamatsu PMT outside (a) and inside its insert (b). The PMT is potted inside its black PVC
        insert to create a watertight seal that can withstand the \SI{6}{\text{atm}} of water
        pressure at the bottom of the pit. A \si{}{\micro}DAQ detailed in \ChapterRef{chap:daq}, is
...
detector simulation (outlined in \SectionRef{sec:chips_monte_carlo_sim}), is shown in
\FigureRef{fig:coverage} alongside the chosen boundaries.
...
classification presented in \SectionRef{sec:cnn_old} and should be revisited in the future with
...
\SI{1.3}{\text{m}} high region allows for the rejection of predominantly downward going cosmic
...
through a \SI{500}{\text{m}} long flexible PVC umbilical resting on the pit floor. The umbilical
...
\SI{30}{\text{m}} photon attenuation length for adequate light collection. Therefore, a high
volume pump is used to continually pull the internal detector water through a \SI{500}{\text{m}}
...
consists of a \SI{51}{\text{cm}} long \SI{10}{\micro\text{m}} carbon block filter followed by a
\SI{51}{\text{cm}} long \SI{0.5}{\micro\text{m}} polypropylene filter, as shown in
\FigureRef{fig:filtration}. This configuration is found to achieve a photon attenuation length of
\SI{133\pm2}{\text{m}} after approximately two months of constant filtering~\cite{campbell2020}.
...
separation liner, and reduce the module height to \SI{8}{\text{m}}. In October 2019 this version
...
\begin{figure} % COSMIC EVENT DIAGRAM %
    ...
    measured ToT value (described in \SectionRef{sec:daq_hard}). Due to the small attenuation
...
photocathode coverage detailed in \SectionRef{sec:chips_detector_instrumentation}.
...
\SI{50}{\text{m}} at \SI{405}{\text{nm}} is used alongside negligible
...
volume. In this work, the \chipsfive geometry is given a top-cap veto of height \SI{1.3}{\text{m}}
...
inner detector volume, while cosmic vertices are set at \SI{1}{\text{m}} above the detector
...
all hits within \SI{200}{\text{ns}} windows are grouped to form a single recorded hit, with a
...
input. Single muons are generated at sea level by CRY within a \SI{1}{\text{km}} by
\SI{1}{\text{km}} area, with the detector at it's centre. Note that only single muon events (the
dominant cosmic component) are considered for simplicity.

Assuming a \chipsfive overburden of \SI{50}{\text{m}} and a \SI{2.2}{\MeV/\text{cm}^{2}} muon
energy loss in water as suggested by \ReferenceRef{klimushin2001}, the muon parameters are updated
to estimate their values at \SI{1}{\text{m}} above the detector. All muons whose path do not cross
...
equal to \SI{24}{\text{m}} are used to estimate a \chipsfive cosmic muon rate of
\SI{11.8}{\text{KHz}} at \SI{50}{\text{m}} of overburden.

Given the \SI{10}{\micro\text{s}} long \numi beam spill occurring every \SI{1.33}{\text{seconds}},
an in spill cosmic rate of $2.1\pm0.2$ million events per year is calculated with an in spill
occupancy of 9\%. Considering a typical event takes $\sim$\SI{100}{\text{ns}} to unfold, there is
...
\begin{figure} % COSMIC RATE DIAGRAM %
    ...
    \SI{24}{\text{m}} in height and \SI{24}{\text{m}} in diameter \chips detector module. Shown
    are single (blue) and double (red) exponential function fits to work originally conducted in
    \ReferenceRef{bugaev1998}. Importantly, in the region of interest at $h_{0}\sim$
    \SI{50}{\text{m}} both models are consistent. Figure taken from \ReferenceRef{son2013}.}
