...
\SectionRef{sec:results_eval_beam}. Therefore, the expected cosmic muon contamination of both beam
...
\begin{table}
    ...
    {Table showing CC $\nu_{e}$ selected event numbers and corresponding efficiencies for the
        ...
        cut (Cuts), in addition to the numbers after the FOM-$\nu_e$ optimised selection for which
        the efficiencies relative to the total number of events are shown. Both the appeared
...
\begin{table}
    ...
    {Table showing CC $\nu_{\mu}$ selected event numbers and corresponding efficiencies for the
        ...
        selection for which the efficiencies relative to the total number of events are shown. The
...
by interaction type are shown in \FigureRef{fig:final_energy_frac_split}. Furthermore, the
neutrino energy resolutions derived from the Full Width Half Maximum (FWHM) of these plots for
both CC $\nu_{e}$ and CC $\nu_{\mu}$ events is shown in \TableRef{tab:energy_resolutions}.
...
\FigureRef{fig:final_energy_frac} in \SI{1}{GeV} wide true neutrino energy bins. These values are
shown in \FigureRef{fig:final_energy_nuel} and \FigureRef{fig:final_energy_numu} for CC $\nu_{e}$
...
bins of size \SI{2.5}{\text{m}} in $\theta$ by \SI{2.0}{\text{m}} in $\phi$ within the \chipsfive
...
With an effective calibration, a realistic discrepancy in hit times of less than \SI{1}{\text{ns}}
...
HZC PMTs produce a dark noise rate of $\sim$\SI{1}{\text{KHz}}. Therefore, only $\sim0.02\%$ of
event map bins are expected to be affected by PMT noise throughout a typical \SI{100}{\text{ns}}
