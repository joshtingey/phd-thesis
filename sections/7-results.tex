...
scaled by a factor randomly drawn from a normal distribution with mean of $\mu$ and a standard
deviation of $\sigma$. This methodology is used instead of an absolute scaling as a bin with a
large collected charge should be proportionally modified more than a low charge bin to be
accurate.
...
Fig.~\ref{fig:calib_charge_energy}.
...
- A non negligible difference is seen for beam classification
- Surprisingly the energy estimation is resilient, given you would assume the exact amount of
...
\begin{figure} % UNIFORM TRAINING SAMPLE DIAGRAM %
    ...
    \caption[Number of training events per category for the uniform beam classification network
        training sample] {Number of training events per category for the uniform beam classification
        network training sample. All beam event interaction types are shown.}
