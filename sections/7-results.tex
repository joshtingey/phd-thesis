...
presented in the previous section is clear from the cosmic rejection visualisation. Conversely,
for the beam classification network, the separation is weaker, with major overlap between
...
of size \unit{2.5}{\mathrm{m}} in $\theta$ by \unit{2.0}{\mathrm{m}} in $\phi$ within the
...
deviation of $\sigma$ nanoseconds. The cases when $\sigma=0~\mathrm{ns}$ (no smearing),
$\sigma=2~\mathrm{ns}$, and $\sigma=5~\mathrm{ns}$ are considered. The actual smearing values used
...
\unit{1}{\mathrm{ns}} should be expected. Given this, it is promising to observe that even for the
smearing of $\sigma=2~\mathrm{ns}$ the beam classification performance change is minimal. At
$\sigma=5~\mathrm{ns}$ more significant degradation starts to occur; however, there is no dramatic
...
\unit{88}{\mathrm{mm}} HZC PMTs produce a dark noise rate of $\sim$\unit{1}{\mathrm{KHz}}.
Therefore, only $\sim0.02\%$ of event map bins are expected to be affected by PMT noise throughout
a typical \unit{100}{\mathrm{ns}} event. Given this, excellent beam classification robustness is
