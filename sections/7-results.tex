...
Another common concern with CNNs is that they do not generalise well under distributional shifts
within the input data. 

In the context of the CNNs trained here, this could mean that if the Monte
Carlo generated events differ from those measured by a real \chipsfive detector, confidence in any
of the network output is questionable.

Another common concern with CNNs is that they do not generalise well under distributional shifts
within the input data. In the context of this work if the Monte Carlo generated training and
performance evaluation events differ from those measured by a real \chipsfive detector, confidence
in any of the network outputs can be brought into question. A strong calibration procedure and
corresponding simulation improvements can always go some way to ensuring any discrepancy is
minimised, but a difference will always exist.

The inputs can be characterised as being dependent on three things, the hit times, the hit charges
and their positions. The 

To quantify this effect, three studies are presented here to determine how robust the trained CNNs
are to changes within the input events. Firstly, the smearing of hit times. Secondly, the smearing
and shifting of hit charges. Finally, the addition of random noise.

position is obviously robust as our bins are 2.5m is theta and 2m in phi approx (when viewed from
the centre of the detector), this means the absolute position of PMTs is not very important in the
context of the networks trained here, maybe future work could look at making the images bigger,
but did that hee to speed up optimisation process and make training not take too long. - Mention
the calibration - Three short robustness studies are presented here for smearing the hit times,
smearing and shifting the charge and the addition of random noise to the input. - Robustness paper
in ~\cite{djolonga2020}

Here we present modifications to the input event maps to see how robust the performance is. T

\subsection*{Time smearing} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

- Put in ROC and PRC description
- Receiver operating characteristic and precision-recall
...
\begin{table} % CALIB TIME COMPARISON METRICS TABLE %
    \begin{tabular}{cccc}
        ...
        \midrule
        Maximum FOM Value        & 0.519                  & 0.513                  & 0.484                  \\
        Highest Score Efficiency & 0.835                  & 0.826                  & 0.798                  \\
        Highest Score Purity     & 0.554                  & 0.556                  & 0.561                  \\
        ROC Integral             & 0.828                  & 0.828                  & 0.826                  \\
        PRC Integral             & 0.756                  & 0.751                  & 0.730                  \\
    \end{tabular}
    \caption[Performance metrics for different levels of hit-time smearing]
    {Performance metrics for different levels of hit-time smearing. The \emph{Highest Score}
        values use the highest scoring output neuron for classification. The ROC and PRC integrals
        are taken from the curves shown in Fig.~\ref{fig:calib_time_nuel_comp_curves}.}
...
\begin{figure} % CALIB TIME NUEL COMP CURVES DIAGRAM %
    ...
    {Receiver operating characteristic (left) and precision-recall (right) curves for different
        levels of hit-time smearing. The $\sigma=0~\mathrm{ns}$ curves are shown by the solid
        lines, $\sigma=2~\mathrm{ns}$ curves by the dashed lines, and $\sigma=5~\mathrm{ns}$
        curves by the dotted lines.}
...
\begin{table} % CALIB CHARGE COMPARISON METRICS TABLE %
    \begin{tabular}{cccccc}
        ...
        \midrule
        Max FOM        & 0.519            & 0.517            & 0.474            & 0.476            & 0.414            \\
        High Score Eff & 0.835            & 0.807            & 0.725            & 0.744            & 0.630            \\
        High Score Pur & 0.554            & 0.596            & 0.645            & 0.626            & 0.649            \\
        ROC Integral   & 0.828            & 0.827            & 0.820            & 0.821            & 0.812            \\
        PRC Integral   & 0.756            & 0.749            & 0.710            & 0.710            & 0.647            \\
    \end{tabular}
    \caption[Performance metrics for different levels of hit-charge smearing and shifting]
    {Performance metrics for different levels of hit-charge smearing and shifting. The
        \emph{High Score} values use the highest scoring output neuron for classification.}
    \label{tab:calib_time}
\end{table}
...
\begin{table} % CALIB NOISE COMPARISON METRICS TABLE %
    \begin{tabular}{cccc}
        Metric                   & $0.0\%$ & $2.3\%$ & $6.7\%$ \\
        \midrule
        Maximum FOM Value        & 0.519   & 0.503   & 0.316   \\
        Highest Score Efficiency & 0.835   & 0.824   & 0.717   \\
        Highest Score Purity     & 0.554   & 0.545   & 0.414   \\
        ROC Integral             & 0.828   & 0.827   & 0.810   \\
        PRC Integral             & 0.756   & 0.740   & 0.559   \\
    \end{tabular}
    \caption[Performance metrics for different levels of noise]
    {Performance metrics for different levels of noise. The \emph{Highest Score}
        values use the highest scoring output neuron for classification.}
    \label{tab:calib_time}
\end{table}
...
\begin{table} % REPR COMPARISON METRICS TABLE %
    ...
    \end{tabular}
    \caption[Performance metrics for three input representations]
    {Performance metrics for the three input representations. The \emph{Highest Score} values use
        the highest scoring output neuron for classification.}
    \label{tab:repr}
\end{table}
...
\begin{table} % SAMPLE COMPARISON METRICS TABLE %
    ...
    \end{tabular}
    \caption[Performance metrics for different training samples]
    {Performance metrics for the different training samples. The \emph{Highest Score} values use
        the highest scoring output neuron for classification.}
    \label{tab:sample}
\end{table}
...
\begin{table}
    ...
    \end{tabular}
    \caption[Performance metrics for different network architectures]
    {Performance metrics for the different network architectures. The \emph{Highest Score} values
        use the highest scoring output neuron for classification.}
    \label{tab:sample}
\end{table}
