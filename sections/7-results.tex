...
\unit{2}{\text{ms}}. When combined with event seeding and map generation, the total time taken
to fully reconstruct and classify a raw event is less than \unit{0.2}{\text{seconds}}.
...
apart in practice. Cosmic muon events are weighted according to the \unit{11.8}{\text{kHz}} and
\unit{50}{\text{m}} of overburden outlined in Section.~\ref{sec:chips_monte_carlo_cosmic}. The
...
measurement statistical error, the value of $\text{efficiency}\times\text{purity}$
...
\begin{figure} % FINAL NUEL EFF CURVES DIAGRAM %
    ...
    \caption[CC $\nu_{e}$ efficiency, purity and $\text{efficiency}\times\text{purity}$]
    {CC $\nu_{e}$ efficiency, purity and $\text{efficiency}\times\text{purity}$ for different
...
\begin{table}
    ...
    {Table showing CC $\nu_{e}$ selected event numbers and corresponding efficiencies for the
        ...
        numbers (Cuts) in addition to the $\text{efficiency}\times\text{purity}$ (FOM)
...
and 0.78 respectively, to maximise $\text{efficiency}\times\text{purity}$, the standard CC
...
A maximum $\text{efficiency}\times\text{purity}$ of 0.132 is achieved; only 25\% the value
...
\begin{figure} % FINAL OLD PID OUTPUTS DIAGRAM %
    ...
    {Distributions of CC $\nu_{e}$ vs CC $\nu_{\mu}$ (left) and CC $\nu_{e}$ vs NC (right) output
        ...
        events. Due to the long reconstruction time required, a smaller, but comparable evaluation
        sample is used here.}
...
\begin{figure} % FINAL NUMU EFF CURVES DIAGRAM %
    ...
    {CC $\nu_{\mu}$ efficiency, purity and $\text{efficiency}\times\text{purity}$ for
...
approximately \unit{50}{\text{cm}} and \unit{70}{\text{cm}} resolutions in this work. Again, a
...
output tasks. Furthermore, there are clear differences between the networks, with distinct
...
Recent work within CNN research has focused on another concern; they tend to not generalise well
under distributional changes within the input data~\cite{djolonga2020}. As the CNNs presented in
this work are trained and evaluated on simulated Monte Carlo events, this effect is of particular
relevance. If the neutrino events used differ from those measured by the real \chipsfive detector,
the network outputs may not be reliable. An effective calibration procedure and corresponding
simulation improvements can ensure any discrepancy is minimised; however, a small distributional
...
\begin{enumerate}
    \item \textbf{Max FOM:} The optimised figure-of-merit
          ($\text{efficiency}\times\text{purity}$) value. As the FOM is proportional to
          $s/\sqrt{s+b}$ which increases linearly with detector exposure, an improvement in the
          FOM value linearly decreases the exposure time required to reach the same physics
          sensitivity.
    \item \textbf{High Score Eff (Pur):} The CC $\nu_{e}$ selection efficiency (purity) using the
          ...
          strategy is used instead of the FOM selection as it is less susceptible to trading off
          between efficiency and purity, making for easier comparison.
    ...
    \item \textbf{PR Integral:} The area under the Precision-Recall (PR) curve; another standard
          ...
          value is varied, PR curves are. A curve closer to the top-right (high signal purity,
          high signal efficiency) signifies a stronger classification performance. For imbalanced
          class frequencies such as those expected within the \chipsfive detector, the PR curve is
          seen as a more reliable visual indicator of performance than the ROC
          curve~\cite{saito2015}.
\end{enumerate}
...
$\sigma=2~\text{ns}$, and $\sigma=5~\text{ns}$ are considered. The actual smearing values used
are scaled for the zero to one input range, and any post smearing out-of-range values are clipped
...
\begin{figure} % CALIB TIME NUEL EFF CURVES DIAGRAM %
    ...
    {CC $\nu_{e}$ efficiency, purity and $\text{efficiency}\times\text{purity}$ at different
        ...
        $\sigma=0~\text{ns}$ curves are shown by the solid lines, $\sigma=2~\text{ns}$ curves
        by the dashed lines, and $\sigma=5~\text{ns}$ curves by the dotted lines.}
...
\begin{figure} % CALIB TIME NUEL COMP CURVES DIAGRAM %
    ...
    \caption[Receiver operating characteristic and precision-recall curves for different levels of
        ...
        $\sigma=2~\text{ns}$ curves by the dashed lines, and $\sigma=5~\text{ns}$ curves by
        the dotted lines.}
...
The smearing and shifting of input hit charges behaves as expected and produces no significant
...
hough-height inputs. Given this, the relatively extensive modifications made here (40\% shifts,
for example) still produce only small reductions in beam classification performance; therefore, a
...
the overall relative event topology remaining consistent under a significant shift compared to
when greatly smeared.
...
trained CNN) relies principally on the counting of the deposited charge for each event, as is the
case for most experimental particle physics energy predictions. Therefore, any systematic shift in
the measured input data is always expected to bias the output prediction heavily. However, for
...
that expected within \chipsfive. Testing has shown that at room temperate the
\unit{88}{\text{mm}} HZC PMTs produce a dark noise rate of $\sim$\unit{1}{\text{KHz}}.
Therefore, only $\sim0.02\%$ of event map bins are expected to be affected by PMT noise throughout
a typical \unit{100}{\text{ns}} event. Given this, excellent beam classification robustness is
observed, with the small reduction in performance seen for the $2.3\%$ chance case indicating any
realistic output changes would be negligible.
...
performance, specifically the primary CC $\nu_{e}$ selection is used here for comparison; however,
these findings also translate to cosmic rejection and energy estimation performance.
...
Super-Kamiokande CNN studies. This mapping attempts to equally distribute PMT density across the
whole two-dimensional input representation. The PMT positions in cylindrical coordinates
($\rho,\phi,z$) are mapped to two dimensions, $X_{+}$ and $X_{-}$ using
...
event maps are used as input, as the hough-height map can only be generated from the seed
estimated interaction vertex position. An example CC resonant $\nu_{e}$ event as each
...
interaction vertex position. The emitted Cherenkov radiation and resulting ring are viewed as
accurate to their physical topology and distorted less by the cylindrical surface of the detector.
This clarity allows the CNN to solely learn the underlying topological differences between event
types instead of also having to untangle detector induced distortions.
...
A significant reduction in performance is observed when using either origin view representations,
backing up the qualitative description above. Interestingly, both origin view representations
result in a very similar performance, showing that uniform PMT distribution across the input
representation does not provide improvements. Future work should consider other methods for
removing detector induced distortions, such as smearing of hits across nearby bins and an improved
interaction vertex position estimation.
...
cases, it can be assumed that as the training sample tends towards the expected flux, the
performance increases. This finding highlights a key feature of CNNs; they play the game of
statistics and only statistics, making them less smart than they may first appear.
...
Section.~\ref{sec:results_eval_beam}. Future work should consider the use of a different loss
function to promote learning of less common event classes, as to maximise the physics sensitivity
rather than simple accuracy.

\subsection{Alternative architectures} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:results_alt_arch} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The choice of CNN architecture is found not significantly to impact performance. Four different
...
architecture~\cite{he2016_improved}, and an Inception-ResNet based architecture\cite{szegedy2016}.
Each is modified to fit the standard chipsnet pattern with separate initial branches for each
event map followed by a series of combined layers. For a fair comparison, each network is scaled
so that they have approximately the same number of trainable parameters as the default VGG based
architecture.

A beam classification network is trained using each network architecture with individual SHERPA
optimised hyperparameters. For each network architecture, the classification performance metrics
are presented in Table.~\ref{tab:sample} along with the number of trainable parameters (Number of
pars) and the time taken in milliseconds to complete a single training iteration step (Iter time).
...
leading to drastically reduced overall training times.

The other architectures are primarily the result of advances that improve the classification of
real-world objects, such as cars, trees, and dogs, with image sizes typically $256\times256$
pixels or greater. Therefore, it is likely that the more subtle task of learning slight changes in
Cherenkov ring topology with a relatively limited number of classes on $64\times64$ images does
not require the advanced feature representation. In this case, the increased complexity models
introduce additional bias acting to decrease performance, instead of providing additional
