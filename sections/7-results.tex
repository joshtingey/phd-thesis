...
outputs are questionable. An effective calibration procedure and corresponding simulation
...
detector. As this binning is much larger than the actual distance between PMTs, resolution of
individual PMTs is not possible. Consequently, changes in only the PMT hit times and hit charges
are considered in three studies: the smearing of hit times, the smearing and shifting of hit
charges, and the addition of random noise.
...
The smearing of hit times is found to have a negligible impact on performance. For every event,
...
Fig.~\ref{fig:calib_time_nuel_comp_curves}. A negligible affect on cosmic rejection and energy
estimation performance is observed and so not shown.
...
Relative to the $\sim$\unit{2}{\mathrm{ns}} time resolution of the \unit{88}{\mathrm{mm}} HZC PMTs
(and associated readout electronics) used within \chipsfive an additional $\sigma=2~\mathrm{ns}$
hit-time smearing is significant. Given an effective calibration and reasonable simulation, a
smaller difference is highly likely. Therefore, it is promising to observe that for
$\sigma=2~\mathrm{ns}$ smearing the beam classification network is robust, with only a small
reduction in performance. Interestingly, the \emph{High Score Pur} is seen to increase with
increased hit-time smearing. This trend indicates that the CC $\nu_{e}$ signal is proportionally
more robust to hit-time input changes than the background.

\subsection*{Charge smearing and shifting} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

For every event, each hit-charge bin for which there is an entry not equal to zero is modified
using an absolute charge randomly generated using a normal distribution with a mean of $\mu$ and a
standard deviation of $\sigma$ photoelectrons. Only those bins that contain a hit are modified.
Alongside the default case when $\mu=0$ and $\sigma=0$, two smearing and two shifting cases are
considered: $\mu=0,\sigma=1$ and $\mu=0,\sigma=2$ for smearing, and $\mu=1,\sigma=1$ and
$\mu=2,\sigma=1$ for shifting. Note that for the shifting cases a standard deviation of
$1~\mathrm{p.e}$ is used to introduce randomness.
...
shifting cases shown in Fig.~\ref{fig:calib_charge_shift_nuel_eff_curves}. All the classification
...
Fig.~\ref{fig:calib_charge_abs_energy}.
...
\begin{figure} % CALIB CHARGE ABS ENERGY DIAGRAM %
    ...
    {Distributions of (reco-true)/true CC QEL $\nu_{e}$ energies for different levels of
        hit-charge smearing and shifting. Shown are the different smearing levels (left) and the
        different shifting levels (right). Standard deviations found from fits made to each
        distribution are shown in brackets.}
    \label{fig:calib_charge_abs_energy}
\end{figure}

- Shift and smearing should be smaller hopefully
- A non negligable difference is seen for beam classification
- Suprisingly the energy estimation is resiliant, given you would assume the exact amount of
collected charge was important.
- Increased purity and reduced efficiency, lower FOM overall!
- Need to understand why this shift is happening!

\begin{figure} % CALIB CHARGE SHIFT NUEL OUTPUTS DIAGRAM %
    \includegraphics[width=0.7\textwidth]{diagrams/7-results/calib_charge_nuel_outputs.pdf}
    \caption[Distribution of CC $\nu_{e}$ scores when the hit-charge is shifted using $\mu=2~\mathrm{p.e}$]
    {Distribution of \emph{combined category} CC $\nu_{e}$ scores from the trained beam
        classification network for the different event categories when the hit-charge is shifted using
        $\mu=2~\mathrm{p.e}$. For comparison Fig.~\ref{fig:final_beam_nuel_outputs} shows the
        distribution of un-shifted output values.}
    \label{fig:calib_charge_nuel_outputs}
\end{figure}

\subsection*{Random noise} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Random PMT noise is found to have a negligible impact on performance. For each hit-charge bin for
every event map in the evaluation sample a normal distribution with a mean of zero and a standard
deviation of $\mu$ photoelectrons is randomly sampled. If a value greater than that corresponding
to a single photoelectron is returned, the bin charge is incremented by the returned value.
Furthermore, if a hit-charge bin is modified, the corresponding hit-time bin is updated by
randomly sampling a uniform distribution and choosing the smallest of the return value and the
already set bin value. Alongside the default case with no noise ($0.0\%$), values of $\mu$ are
chosen so that either $2.3\%$ or $6.7\%$ of bins are modified by noise for each event.
...
in Table.~\ref{tab:calib_time}. Negligible reduction in cosmic rejection and energy estimation
performance is observed and so not shown here.
...
- This is orders of magnitude more noise than you would expect. Even for 10Khz PMT noise you would
expect $\sim0.2\%$ of PMTs to be impacted by noise within the time frame (100ns) of a typical
event.
- The effect is not huge, so noise should not be a problem for the robustness of the networks.
