...
- The principle goal of event reconstruction in CHIPS is to identigy electron neutrino
...
CHIPS detectors.
- CNN's have been widely applied in various computer vision tasks to solve image recongnition and
analysis problems.
- The core problem in HEP is the correct categorisation of particle interactions
- This is usually done by reconstructing high-level componenets suh as clusters, tracks, showers,
jets and rings. and then summarising these objects energies, directions and shapes, these
wuantities are then fed into k-nearest neighbours, BDTs or MLPs to seperate sign from bkg.
- Prone to failure, mistakes in the reconstruction, and limitation to what has been implemented by
humans.
- computer vision moved away from specifically constructed features to sing ML CNNs to discover
the features.
- Manu HEP problems including water cherenkov detectors essentially result in an 'image' of an
...
- They truly caught the attention of the wider ML communityr in 2012 when A. Krizhevsky, I,
...
- Such was the rapid pace of advance afterwards that the ResNet model acheived a 3.57\percent
error just three years later.
- Many high level libraries have now been formed, predominently led by Tensorflow (from google)
and pyTorch (from facebook) making it easier to quickly code and implemenetd DNNs.
...
that have been used by the \chips project up till now. This is key for two main reasons. Firstly,
to add context to the performance comparison with the new convolutional neural network approach
...
Ref.~\cite{blake2016} and Ref.~\cite{perch2017}.
...
For the actual minimisation process, the Minuit2 algorithm contained within ROOT~\cite{brun1997}
is used. At each iteration, the charge and hit time predictions are made, and the negative
logarithm of the likelihood is calculated. The track parameters are then varied before the next
iteration to help minimise the value. Through a series of stages, fixing and then freeing specific
parameters, the minimisation process converges to the best-fit parameters for the given
hypothesis. This process typically takes approximately two minutes on a standard batch farm node.
...
PMTs. Example of these inputs include the following:
\begin{itemize}
...
\end{itemize}
The first three of these are combined into `emission profiles' generated from large samples of
the Monte Carlo simulation. REFER TO THE EMISSION PROFILE PLOTS!!!
...
calibration is, therefore, required to match the simulated data, let alone real data from a live
detector.

Real events within \chips are rarely simple single electron or muons events or even a two-particle
event, such as the two-photon example given previously. In reality, the majority of events contain
multiple final state particles of various types and in various topologies, highlighting the other
main problem with the likelihood-based approach. Requiring a predefined track hypothesis. This
makes it impossible to implement a generalised approach to event reconstruction where no input
hypothesis, susceptible to bias or lacking in required complexity is required to analyse the
events.

\subsection{Particle identification}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:cvn_old_pid} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Two artificial neural networks have been implemented to provide particle identification using
outputs from the likelihood based event reconstruction.

Particle identification is primarily driven by the difference between the log likelihood for

\begin{itemize}
    \item The $\Delta\ln\mathcal{L}$ between electron and muon hypothesis for both time and charge
          components.
    \item The number of hit PMTs ($N_{hits}$).
    \item $\frac{\Delta\ln\mathcal{L}_{charge}}{N_{hits}}$.
    \item The fraction of hits inside, within and outside the central hole of the ring for
    ...
    \item The peak height score of the first and last seeds found by the Hough transform seeding algorithm.
\end{itemize}
...
- These are all hand engineered features, found by trial and error by a human. They are of course
...
- Highlights another problem in that you have to run multiple hypothesis, yet more computation
time in order to run an effective PID.
...
- Muon rings tend to be sharper and exibit a `plateau' shape as they emmit a constant amount of
Cherenkov radiatioon from a single particle over a period of time.
