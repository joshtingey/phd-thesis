...
classification. Both methods are typical of the mainstream approach used by the majority of water
Cherenkov neutrino experiments. A prime example is the fiTQun algorithm developed for the
Super-Kamiokande detector, which is used for both atmospheric~\cite{jiang2019} and
...
Additionally, the likelihood-based approach requires a predefined track hypothesis. Real events
...
nanoseconds) across all PMTs mapped to that bin. Each hit-time map bin is additionally corrected
so that the first hit time across all bins is at zero.
...
\chipsfive geometry. Roughly $1/3^{rd}$ of events are $\nu_{\mu}$ beam events, $1/3^{rd}$
$\nu_{e}$ beam events, and $1/3^{rd}$ cosmic muon events, the counts of which are shown in
...
preselection applied as this is found to be the best for cosmic rejection performance. The full
...
simulation and event generation methods outlined in Section.~\ref{sec:chips_monte_carlo} with a
\chipsfive geometry. Roughly half the events are $\nu_{\mu}$ beam events, with the other half
being $\nu_{e}$ beam events, as shown in Fig.~\ref{fig:beam_training_sample}. All events (both
$\nu_{\mu}$ and $\nu_{e}$) are generated using the expected \chipsfive $\nu_{\mu}$ energy spectrum
to closely mimic the dominant $\nu_{\mu}$ beam component and appeared $\nu_{e}$ signal. All events
are used for training with no preselection applied as this is found to be the best for beam
classification performance, especially NC rejection. The full sample is split 95\% to 5\% for
