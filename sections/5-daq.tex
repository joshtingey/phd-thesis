...
charge, corresponding to approximately ten incident photons per event for the PMTs used by
\chipsfive. However, as the maximum expected incident photons within \chipsfive are relatively low
(only a few), the impact of the ToT non-linearity is small, and the values are very consistent
with those recorded using an ADC. Furthermore, ToT electronics are simpler and notably cheaper.
