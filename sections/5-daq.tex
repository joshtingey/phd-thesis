...
category 5 cables with RJ45 connectors, as shown in \FigureRef{fig:nikhef_plane}. Both Calamares
...
\FigureRef{fig:madison_plane}. Standard category 5 cables with RJ45 connectors are used for these
...
the standard networking switch (these connections are not present within \FigureRef{fig:hut_daq}).
...
of the CLB software can be found in \ReferenceRef{aiello2019} while the Beaglebone and $\micro$DAQ
software implementation can be found at \ReferenceRef{microdaq2020}.
...
processed on separate threads) as is shown within \FigureRef{fig:daq_software}, expressed in terms
...
\begin{itemize}
    ...
    detailed further in \SectionRef{sec:daq_soft_hits}.
