...
\begin{itemize}
    ...
    for \textsc{Pom} level processing and logic instead of expensive CLBs. Not only are they cheap
    but Beaglebones provide a fully configurable general-purpose Linux machine that can easily be
    enhanced with new features once deployed. This is in comparison to CLBs which can not be
    reconfigured once installed (in the water restricted \chips case).
...
power directly from the master supply.
