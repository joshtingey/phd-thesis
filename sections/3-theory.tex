...
as corresponding to the parameters they encompass: $\Delta m^{2}_{21}$ and $\theta_{12}$ for the
solar sector, $\Delta m^{2}_{32}$ and $\theta_{23}$ for the atmospheric sector, and $\theta_{13}$
for the reactor sector.

During the last couple of decades the focus of experiments in each of these sectors has shifted
from the discovery of neutrino fundamentals to precision measurement. This has led to a
corresponding shift in using increasingly abundant neutrino sources and larger and larger
detectors. Below we outline the current status, open questions, and future for neutrino physics.

\subsection{Current status} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:theory_status_current} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Global fits to neutrino oscillation data aggregate the latest results from all neutrino sectors to
constrain the neutrino oscillation parameters. The best fit results from one such global fit,
NuFIT~\cite{esteban2019,esteban2020}, are shown in Fig.~\ref{fig:best_fit}. A detailed overview of
this and other global fits are given in The Particle Data Group's 2020 Review of Particle
Physics~\cite{particle2020}.
...
The latest results from experiments studying the solar sector parameters yield
$\theta_{12}\approx34^{\circ}$ with about $1.5^{\circ}$ of uncertainty, and $\Delta
    m^{2}_{21}\approx7.4\times10^{-5}\eV^{2}$ with about $0.4\times10^{-5}\eV^{2}$ of uncertainty.

These results primarily come from a range of historic and current experiments studying neutrinos
from the sun: the radiochemical chlorine based Homestake~\cite{cleveland1998}, and gallium based
GALLEX~\cite{kaether2010} and SAGE~\cite{abdurashitov2009}; the deuterium Cherenkov
SNO~\cite{maneira2011}; the water Cherenkov Super-Kamiokande~\cite{hosaka2006, cravens2008,
    abe2011_super, nakano2017}; and the liquid scintillator detector Borexino~\cite{bellini2011,
    Bellini2010, bellini2014}. Many of which were mentioned in Section.~\ref{sec:theory_history_osc}
originally designed to study the solar neutrino problem.

Additionally, the KamLAND experiment has probed the solar sector in a terrestrial setting using
reactor antineutrinos~\cite{gando2011}. By removing the influence of the MSW effect (caused by the
high electron density of the sun) impacting neutrino from the sun, KamLAND was able to remove any
ambiguity in the solar neutrino parameters. It did this by locating a \unit{1}{\mathrm{kton}}
liquid scintillator detector between several nuclear reactors.

\subsubsection*{Atmospheric parameters: $\theta_{23}$ and $\Delta m^{2}_{32}$} %%%%%%%%%%%%%%%%%%%

The latest results from experiments studying the atmospheric sector parameters yield
$\theta_{23}\approx48^{\circ}$ with about $2^{\circ}$ of uncertainty, and $\Delta
    m^{2}_{32}\approx2.45\times10^{-3}\eV^{2}$ with about $0.03\times10^{-3}\eV^{2}$ of uncertainty.

These results firstly come from experiments studying traditional atmospheric neutrinos generated
in the upper atmosphere by cosmic rays.

These results come from two sources. Firstly, experiments studying traditional atmospheric
neutrinos generated in the earth's upper atmosphere by cosmic rays. These include the previously
mentioned Super-Kamiokande~\cite{abe2018} and the IceCube experiment~\cite{aartsen2015}, a neutrino
observatory based at the Amundsen–Scott South Pole Station in Antarctica, using PMTs embedded deep
within the ice.

Additionally, experiments measuring $P(\nu_{\mu}\rightarrow\nu_{\mu})$ in high intensity muon
neutrino beams over a distance of hundreds of km measure the atmospheric paramters. These include:
...
generated at J-PARC (on the east coast of Japan) with Super-Kamiokande being used as a far detector.


\emph{long baseline accelerator experiments}
...
The latest results from experiments studying the reactor sector yield
$\theta_{13}\approx8.5^{\circ}$ with about $0.25^{\circ}$ of uncertainty.
