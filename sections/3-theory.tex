...
surrounding the neutrino as a means to motivate the \chips R\&D project.
...
\begin{figure} % Z-RESONANCE DIAGRAM %
    ...
    {Combined hadron production cross-section measurements around the $Z^{0}$ resonance from the
        ...
        well. Figure taken from Ref.~\cite{electroweak2006}.}
...
\begin{figure} % XSEC NUEL DIAGRAM %
    ...
    {\textsc{Genie} cross-sections on oxygen divided by neutrino energy for CC $\nu_{e}$ (left)
        ...
        cross-sections are smaller than their CC counterparts. See Ref.~\cite{andreopoulos2015}
...
via the first term in Eq.~\ref{eq:chips_prob} allows for the octant to be resolved. Current
...
Eq.~\ref{eq:chips_prob}. Furthermore, determining the hierarchy is also possible using
...
term in Eq.~\ref{eq:chips_prob} is the most promising channel for measurement. The
