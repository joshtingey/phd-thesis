...
detector of three \unit{1500}{\litre} liquid scintillator tanks and two \unit{200}{\litre} cadmium
...
target produced charged pions which then decayed into a beam of muons and neutrinos. After passing
through steel and lead absorbers to remove the muons, neutrino interactions were detected in a
series of spark chambers.
...
\unit{400000}{\litre} of the dry cleaning fluid, perchloroethylene ($C_{4}Cl_{8}$). Its goal
was to measure the solar electron neutrino flux incident upon the earth via the interaction:
...
A neutrino deficit was also observed indirectly in the atmospheric sector. Neutrinos generated
from cosmic rays in the atmosphere formed a key background to the Kamiokande and IMD experiments,
both designed to measure proton decay primarily. When evaluating the background, they observed a
deficit in the number of muon neutrinos compared to electron neutrinos~\cite{hirata1988,
    becker1992}. The successor to the Kamiokande experiment Super-Kamiokande also measured the same
...
gauge theory obeying the local gauge symmetries of $U(1) \times SU(2) \times SU(3)$. The
...
provide the best limit on the combined sum of the neutrino masses at $\sum m_{\nu} <
    0.12~\eV$~\cite{planck2018}. The KATRIN experiment has been able to make model-independent direct
measurements of the upper limit by looking at the energy spectrum of electrons emitted from beta
decay, giving a result of $m_{\nu} < 1.1~\eV$~\cite{aker2019}.

The Standard Model does not strictly rule out massive neutrinos which may either be Dirac or
Majorana particles. If neutrinos are Dirac particles, they acquire their mass via a Yukawa
coupling just like the other fermions. This mixes both left-handed and right-handed fields,
...
mass states. Additionally, the relativistic limit allows the simplification $t = L$, where $L$ is
the distance from neutrino creation to detection. Combined, the oscillation probability can be
written as,
...
oscillation of the flavour state. A fixed experimental location and neutrino source define a
...
As outlined previously, there are two types of weak neutrino interactions with nucleons.
Charged-current (CC) and neutral-current (NC) occurring via the exchange of a $Z^{0}$ or a
$W^{\pm}$ respectively. During a CC interaction, the neutrino is transformed into a charged lepton
of the same flavour, conserving charge, flavour, and lepton number as shown in
...
interactions are background as there is no way to identify the original neutrino without a charged
lepton and when the cross-sections are constant across all neutrino flavours.
...
\begin{itemize}
    \item \textbf{Quasi-Elastic Scattering (QE)} is the dominant channel for energies below
          \unit{1}{\GeV} and involves the neutrino scattering off the entire nucleon. In the CC
          neutrino case, this converts the target neutron to a proton which is reversed for
    ...
    \item \textbf{Meson Exchange Current (MEC)} provides an additional contribution to the range
          dominated by QE interactions. The interaction involves two nucleons and produces two
          protons in the final state. Sometimes referred to as the \emph{2p2h} interaction it has
          become essential for explaining the discrepancy between theory and observations within
          many modern experiments and is now included by default in popular event
...
The standard $\nu_{e}$ cross-sections included with the GENIE event
generator~\cite{andreopoulos2009, andreopoulos2015} for all the above processes are shown in
Fig.~\ref{fig:xsec_nu_e_O16} for both CC and NC interactions. These also apply approximately to
$\nu_{\mu}$ interactions, however, for $\nu_{\tau}$ interactions the cross-section differ
considerably due to the large $\tau$ lepton mass as shown in Fig.~\ref{fig:tau_comparison}.
...
- Will talk about the main past, present and future experiments for each sector
- Quickly move onto the stuff that is relevant for CHIPS (long-baseline accelerator beam exp)
