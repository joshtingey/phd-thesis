...
surrounding the neutrino to motivate the \chips R\&D project.
...
\EquationRef{eq:inverse_beta_decay}. In an underground room of the reactor building, a
`club-sandwich' detector was constructed containing three \SI{1500}{\text{litre}} liquid
scintillator tanks interspersed with two \SI{200}{\text{litre}} cadmium doped water target tanks.
...
1990s made such precise measurements (see \FigureRef{fig:z_resonance}), indicating that the number
...
\begin{figure} % Z-RESONANCE DIAGRAM %
    ...
    {Combined hadron production cross-section measurements around the $Z^{0}$ resonance from the
        ...
        well. Figure taken from \ReferenceRef{electroweak2006}.}
...
At the Homestake mine in the 1960s, a \SI{400000}{\text{litre}} tank, placed \SI{1.5}{\text{km}}
...
\begin{equation} % HOMESTAKE CHLORINE EQUATION %
    ...
    \label{eq:homestake}
\end{equation} %
This process (\EquationRef{eq:homestake}) allowed the neutrino flux to convert the chlorine
...
pivotal background to both the Kamiokande and IMD experiments, which were primarily designed to
...
SNO consisted of a \SI{1}{\text{kt}} tank of deuterium (heavy water), equipped with 9500
...
particles, along with their various properties, are outlined in \FigureRef{fig:sm}. Additionally,
...
\begin{figure} % STANDARD MODEL DIAGRAM %
    ...
    {The particles of the Standard Model. Figure taken from \ReferenceRef{wiki2020}.}
    \label{fig:sm}
\end{figure}
...
electromagnetic force (affecting all charged particles), the gluons carry the strong force
...
the result from SNO amongst others detailed in \SectionRef{sec:theory_status}. These oscillations
...
From inspecting \EquationRef{eq:osc_prob_2}, the superposition of mass eigenstates is seen to
...
interact via the CC channel as shown in \FigureRef{fig:coherent_scattering}. Consequently, only
...
\begin{figure} % NUEL COHERENT SCATTERING DIAGRAM %
    \feynmandiagram[vertical'=a to b] {i1 [particle=\(\nu_{e}\)]-- [fermion] a-- [fermion] f1
    [particle=\(e^{-}\)], a -- [boson,edge label=\(W^{+}\)] b, i2 [particle=\(e^{-}\)]-- [fermion]
    b-- [fermion] f2 [particle=\(\nu_{e}\)],};
...
As outlined in \SectionRef{sec:theory_oscillations_matter}, there are two types of weak neutrino
...
number as shown in \FigureRef{fig:cc_interaction}. Contrastingly, during a NC interaction, there
is no charge or flavour exchange, and the neutrino continues into the final state as shown in
\FigureRef{fig:nc_interaction}.

\begin{figure} % CC DIAGRAM %
    \feynmandiagram[vertical'=a to b] {i1 [particle=\(\nu_{l}\)]-- [fermion] a-- [fermion] f1
    [particle=\(l^{-}\)], a -- [boson,edge label=\(W^{+}\)] b, i2 [particle=\(n\)]-- [fermion] b--
    [fermion] f2 [particle=\(P^{+}\)],};
...
\begin{figure} % NC DIAGRAM %
    \feynmandiagram[vertical'=a to b] {i1 [particle=\(\nu_{l}\)]-- [fermion] a-- [fermion] f1
    [particle=\(\nu_{l}\)], a -- [boson,edge label=\(Z^{0}\)] b, i2 [particle=\(N\)]-- [fermion]
...
\begin{itemize}
    ...
    \item \textbf{Resonant pion production (Res)} is the dominant channel between 1 and
          ...
          \SectionRef{sec:chips_concept_cherenkov} forms an incredibly tricky component of the
...
\FigureRef{fig:xsec_nu_e_O16} for both CC and NC interactions. For $\nu_{\tau}$ interactions the
CC cross-sections differ considerably due to the large tau lepton mass, as shown in
\FigureRef{fig:tau_comparison}, only becoming non-negligible for energies above
$\sim\SI{5}{\GeV}$.
...
\begin{figure} % XSEC NUEL DIAGRAM %
    ...
    {\textsc{Genie} cross-sections on oxygen divided by neutrino energy for CC $\nu_{e}$ (left)
        and NC $\nu_{e}$ (right). Shown are the total and contributing process cross-sections.
        Note how the total cross-section approaches a linear dependence on energy and the NC
        cross-sections are smaller than their CC counterparts. See \ReferenceRef{andreopoulos2015}
...
\begin{figure} % TAU CROSS-SECTION DIAGRAM %
    ...
    {The total CC interaction cross-section per nucleon divided by the neutrino energy for
        $\nu_{\mu}$ and $\nu_{\tau}$. Figure taken from \ReferenceRef{formaggio2012}.}
...
The three-component representation of the PMNS matrix presented in \EquationRef{eq:matrix} splits
...
\FigureRef{fig:best_fit}. A detailed overview of other global fits is given in The Particle Data
Group's 2020 Review of Particle Physics (see \ReferenceRef{particle2020}). In summary, all mixing
...
\begin{figure} % BEST FIT DIAGRAM %
    ...
    {Three-flavour results from the NuFIT v5.0 global fit as of July 2020, showing both 1$\sigma$
        ...
        does not. Figure taken from \ReferenceRef{esteban2020}.}
    \label{fig:best_fit}
\end{figure}
...
\SectionRef{sec:theory_history_osc}, originally involved in studying the solar neutrino problem.

Additionally, by locating a \SI{1}{\text{kt}} liquid scintillator detector between several nuclear
...
normal hierarchy as discussed in \SectionRef{sec:theory_status_open}) with
...
Whilst, Super-Kamiokande is a large \SI{50}{\text{kt}} water Cherenkov detector, equipped with
11000 PMTs, and located \SI{1}{\text{km}} underground in Gifu Prefecture, Japan.

Furthermore, experiments measuring $P(\nu_{\mu}\rightarrow\nu_{\mu})$ using accelerator beam
...
the octant. Although $\theta_{23}$ is easily measured be studying $\nu_{\mu}$ disappearance, a
degeneracy in the oscillation probability removes the ability to determine the octant. However,
$\nu_{e}$ appearance via the first term in \EquationRef{eq:chips_prob} allows for the octant to be
resolved. Current results suggest the $\theta_{23}>45^{\circ}$ octant is preferred; however, a
...
$m_1<m_2<m_3$, known as the \emph{normal hierarchy}, or $m_3<m_1<m_2$, known as the \emph{inverted
hierarchy}, as illustrated in \FigureRef{fig:hierarchy}. Resolving this ambiguity will
...
\EquationRef{eq:chips_prob}. Alternatively, determining the hierarchy is also possible using
...
term in \EquationRef{eq:chips_prob} is the most promising channel for measurement. The oscillation
probabilities are seen to change significantly with $\delta_{CP}$, as shown in
\FigureRef{fig:osc_cp_probs}. Current results from both \nova~\cite{acero2019} and
...
\begin{figure} % OSCILLATION CP PROBS DIAGRAM %
    ...
    \SI{712}{\text{km}}. The oscillations are show for three different values of $\delta_{CP}$.}
...
DUNE~\cite{acciarri2016, abi2020}, will consist of four \SI{10}{\text{kt}} liquid argon time
projection chambers (LArTPCs) located \SI{1.5}{\text{km}} underground at the Sanford Underground
Research Facility, South Dakota. This location is the same as the Homestake mine chlorine
experiment previously mentioned in \SectionRef{sec:theory_history_osc}. LArTPCs have been chosen
...
(\SI{1285}{\text{km}} away) named the Long-Baseline Neutrino Facility (LBNF). Replacing the
current \SI{700}{\text{kW}} \numi (Neutrinos at the Main Injector) beam, LBNF is initially planned
to run at \SI{1.2}{\text{MW}}, but will eventually reach \SI{2.4}{\text{MW}}.
...
water Cherenkov detectors each \SI{60}{\text{m}} in height and \SI{74}{\text{m}} in diameter. Both
will be built under \SI{650}{\text{m}}  of rock at the Tochibora mine in Japan, close to the
current Super-Kamiokande location. New high-efficiency photomultiplier tubes will be used,
...
will increase the beam power from \SI{400}{\text{kW}} to \SI{1.3}{\text{MW}}. In total, the
Hyper-Kamiokande project is expected to cost nearly \$700 million~\cite{nature_budget}.