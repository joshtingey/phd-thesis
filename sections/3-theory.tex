...
photomultiplier tubes then measured the two characteristic signals for the interaction. Firstly,
the positron annihilation, followed shortly afterwards by a gamma-ray burst from the neutron
capture on cadmium.
...
neutrino was thought extremely likely. The Fermilab based DONUT experiment finally discovered this
...
After analysis, the number of electron neutrino interactions per ${}^{37}Cl$, per second, was
found to be no greater than three~\cite{davis1968}. When compared to the predictions made by the
Standard Solar Model ranging between 4.4 and 22~\cite{bahcall1968}, a deficit was observed. Dubbed
the \emph{solar neutrino problem}, this was initially believed to be due to an unexplained
...
5.3$\sigma$ that electron neutrinos had oscillated to other flavours, whilst the total solar
neutrino flux remained constant.
...
belief is due to the substantial amount of evidence in support of neutrino oscillations which
...
the neutrino is its own anti-particle, which as neutrinos do not carry a charge, is possible.
...
PMNS matrix. Additionally, as a fixed experimental location and neutrino source define a constant
$L/E$, the period of oscillations is determined by the squared mass difference between the flavour
...
shifted from the discovery of neutrino fundamentals to precision measurement. This change has led
to a corresponding shift in using increasingly abundant neutrino sources (such as accelerator
neutrino beams) and larger and larger detectors. Here we outline the current status, open
questions, and future for neutrino physics.
...
\begin{figure} % BEST FIT DIAGRAM %
    ...
    {Three-flavour results from the NuFIT v5.0 global fit as of July 2020, showing both 1$\sigma$
        ...
        normal hierarchy while the second for an inverted hierarchy. Additionally, the lower
        section includes atmospheric neutrino data from the Super-Kamiokande experiment, while the
        upper section does not. Figure taken from Ref.~\cite{esteban2020}.}
    \label{fig:best_fit}
\end{figure}
...
about $1.5^{\circ}$ of uncertainty, and $\Delta m^{2}_{21}\sim7.4\times10^{-5}\eV^{2}$ with about
$0.4\times10^{-5}\eV^{2}$ of uncertainty.
...
Borexino~\cite{bellini2011, bellini2010, bellini2014}. Many of which were mentioned in
...
with about $2^{\circ}$ of uncertainty, and $\Delta m^{2}_{32}\sim2.45\times10^{-3}\eV^{2}$
(assuming normal hierarchy as discussed in Section.~\ref{sec:theory_status_open}) with about
$0.03\times10^{-3}\eV^{2}$ of uncertainty. These results come from one of two primary sources.
...
Additionally, \emph{long-baseline accelerator experiments} measuring
...
best fit results yielding a value of $\theta_{13}\sim8.5^{\circ}$ with about $0.25^{\circ}$ of
...
made by MINOS~\cite{adamson2013_2}, T2k~\cite{abe2013} and \nova~\cite{adamson2016_2}, but with
...
the current \unit{700}{\mathrm{kW}} NuMI beam, LBNF is initially planned to run at
\unit{1.2}{\mathrm{MW}}, but will eventually reach \unit{2.4}{\mathrm{MW}}.
...
This will increase the beam power from \unit{400}{\mathrm{kW}} to \unit{1.3}{\mathrm{MW}}. In
